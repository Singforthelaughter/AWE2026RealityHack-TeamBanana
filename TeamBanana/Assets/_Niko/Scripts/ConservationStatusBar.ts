...
const STATUSES: ConservationStatus[] = [
  ...
  {
    label: "Secure",
    explanation: "Population is stable with no immediate conservation concern.",
    value: 0
  },
  {
    label: "Watch",
    explanation: "Population is stable now but needs continued monitoring.",
    value: 0.25
  },
  {
    label: "Vulnerable",
    explanation: "Population is declining or habitat pressure is increasing.",
    value: 0.5
  },
  {
    label: "Endangered",
    explanation: "Population is at high risk without active protection.",
    value: 0.75
  },
  {
    label: "Critical",
    explanation: "Population is at severe risk and needs urgent protection.",
    value: 1
  }
]
...
export class ConservationStatusBar extends VisualElement {
  ...
  @widget(
    new ComboBoxWidget([
      ...
      new ComboBoxItem("Secure", "Secure"),
      new ComboBoxItem("Watch", "Watch"),
      new ComboBoxItem("Vulnerable", "Vulnerable"),
      new ComboBoxItem("Endangered", "Endangered"),
      new ComboBoxItem("Critical", "Critical")
    ])
  )
  ...
  private normalizeDatabaseStatus(value: string): string {
    ...
    if (normalizedValue === "least concern" || normalizedValue === "secure") {
      return "Secure"
    }

    if (normalizedValue === "near threatened" || normalizedValue === "watch") {
      return "Watch"
    }

    if (normalizedValue === "threatened" || normalizedValue === "vulnerable") {
      return "Vulnerable"
    }

    if (normalizedValue === "endangered") {
      return "Endangered"
    }

    if (normalizedValue === "critically endangered" || normalizedValue === "critical") {
      return "Critical"
    }
  ...
  private isNoInfoStatus(): boolean {
    return this.statusForLabel(this.status).label === "No Info"
  }
