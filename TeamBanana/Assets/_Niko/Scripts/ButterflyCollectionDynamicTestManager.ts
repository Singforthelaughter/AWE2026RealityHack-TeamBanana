...
const PREVIEW_ROTATION =  quat.fromEulerAngles(
    deg(90),
    deg(0),
    deg(90)
)
...
@component
export class ButterflyCollectionDynamicTestManager extends BaseScriptComponent {
  @input
  ...
  public buildTestCollection(): void {
    ...
    const displayEntries = this.createDisplayEntries()
    ...
    if (slotObjects.length < displayEntries.length) {
      this.warn(
        "Only found " +
          slotObjects.length +
          " existing Button slot objects. Dynamic test needs " +
          displayEntries.length +
          ". No fallback lite cards will be generated."
      )
      return
    }
  ...
  private bindButtonHover(button: RectangleButton, cardState: CardInteractionState): void {
    ...
    button.onStateChanged.add((stateName: StateName) => {
      ...
      const isHovered =
        stateName === StateName.hovered ||
        stateName === StateName.triggered ||
  ...
  private applyNewBadgeTemplateTransform(
    badgeObject: SceneObject,
    badgeTransform: Transform
  ): {baseScale: vec3; baseRotation: quat} {
  ...
  private updateNewBadgeAnimations(): void {
    ...
    for (let i = this.newBadgeStates.length - 1; i >= 0; i--) {
      ...
      const animatedScale = new vec3(
        badgeState.baseScale.x * pulse,
        badgeState.baseScale.y * pulse,
        badgeState.baseScale.z
      )
      ...
      const animatedOffset = animatedRotation.multiplyVec3(
        new vec3(
          badgeState.topLeftPivotOffset.x * pulse,
          badgeState.topLeftPivotOffset.y * pulse,
          0
        )
      )
      ...
      badgeState.transform.setLocalScale(
        animatedScale
      )
    }
  }
