...
export class DepthSpatializer extends BaseScriptComponent {
  ...
  private readonly maxDepthReadyRetries = 5
  ...
  private lastDetectionPositions: (vec3 | null)[] = []

  // Missing properties for detection tracking
  private detectionStableFrames: number[] = []
  private lastDetectionCenters: (vec3 | null)[] = []
  ...
  onStart() {
    ...
    if (this.enableContinuousUpdate) {
      ...
      this.delayedEvent.reset(this.continuousUpdateInterval)
      print(`Automatic position testing started with ${this.continuousUpdateInterval}s interval`)
    }
  ...
  private tryUpdatePosition(retryCount: number) {
    ...
    if (!this.depthCache) {
      if (this.debug) {
        print("DepthCache is not available - skipping depth-based spatialization")
      }
      return
    }

    if (typeof this.depthCache.isReady === "function" && !this.depthCache.isReady()) {
      if (this.debug) {
        print(`DepthCache not ready yet: ${this.depthCache.getReadinessDebugString()}`)
      }

      if (retryCount < this.maxDepthReadyRetries) {
        const retryEvent = this.createEvent("DelayedCallbackEvent")
        retryEvent.bind(() => this.tryUpdatePosition(retryCount + 1))
        retryEvent.reset(0.2)
      }
    ...
    try {
      ...
      if (depthFrameID === -1) {
        if (this.debug) {
          print(`Failed to request depth frame - ${this.depthCache.getReadinessDebugString()}`)
        }
      ...
      if (this.debug && this.debugVisualizer) {
  ...
  private depthWorldPosition(cameraFrame: Texture, depthFrameID: number) {
    ...
    if (!detections || detections.length === 0) {
      ...
      for (let i = 0; i < this.detectionStableFrames.length; i++) {
        if (this.detectionStableFrames[i] >= this.stableFramesRequired) {
          ...
          if (this.debug) {
            print(`Maintaining stable detection ${i} (${this.detectionStableFrames[i]} stable frames)`)
          }
        }
      }
    ...
    print(`Processing ${limitedDetections.length} detections (limited from ${detections.length})...`)
  ...
  private processDetectionsWithStaticScene(detections: Detection[], cameraFrame: Texture, depthFrameID: number): void {
    ...
    for (let i = 0; i < detections.length; i++) {
      ...
      if (this.debug) {
        print(`Static Scene Detection ${i} (${detection.label}): Score ${(detection.score * 100).toFixed(1)}%`)
        print(
  ...
  private processDetectionsWithDynamicScene(detections: Detection[], cameraFrame: Texture, depthFrameID: number): void {
    ...
    for (let i = 0; i < detections.length; i++) {
      ...
      if (this.debug) {
        ...
        print(`Visualizing bounding box vertices for detection ${i} (${detection.label})`)
        this.debugVisualizer.visualizeBoundingBoxVertices(detection.bbox, cameraFrame)
      }
      ...
      if (!worldPosition) {
        print(`Detection ${i}: Failed to get world position for pixel (${pixelX.toFixed(1)}, ${pixelY.toFixed(1)})`)
        ...
        if (i < this.detectionInstances.length && this.detectionInstances[i]) {
          if (this.debug) {
            print(`Detection ${i}: Keeping existing instance due to failed world position`)
          }
      ...
      if (!shouldUpdateDetection.updateNeeded) {
        if (this.debug) {
          print(`Detection ${i}: No significant changes detected, maintaining current state`)
        }
      ...
      if (i < this.detectionInstances.length && this.detectionInstances[i]) {
        ...
        // Create new instance
        instance = this.depthPrefab.instantiate(this.getSceneObject())
      ...
      if (this.debug) {
        print(`Detection ${i} (${detection.label}): Score ${(detection.score * 100).toFixed(1)}%`)
        print(
  ...
  private updateStaticSceneDetection(
    ...
    if (!instance) {
      if (this.debug) {
        print(`Static Scene: No pre-instantiated detection at index ${detectionIndex}`)
      }
      return
    }
    ...
    const needsPositionUpdate =
      !state.targetPosition || state.targetPosition.distance(worldPosition) > this.positionUpdateThreshold / 100 // Convert cm to meters
    ...
    if (needsPositionUpdate || needsVertexUpdate) {
      ...
      if (this.debug) {
        print(`Static Scene Detection ${detectionIndex}: Started lerp transition`)
        print(
          `  From: (${currentPosition.x.toFixed(2)}, ${currentPosition.y.toFixed(2)}, ${currentPosition.z.toFixed(2)})`
        )
        print(`  To: (${worldPosition.x.toFixed(2)}, ${worldPosition.y.toFixed(2)}, ${worldPosition.z.toFixed(2)})`)
      }
    }
  ...
  private updateDetectionContainer(
    ...
    if (!detectionContainer) {
      print("Warning: DetectionContainer component not found on detection prefab")
      return
    }
  ...
  private resizeTrackingArrays(targetLength: number): void {
    ...
    const vecArrays = [this.lastDetectionPositions, this.lastDetectionCenters]
    vecArrays.forEach((array) => {
    ...
    while (this.detectionStableFrames.length > targetLength) {
      this.detectionStableFrames.pop()
    }
    while (this.detectionStableFrames.length < targetLength) {
      this.detectionStableFrames.push(0)
    }
  ...
  private calculateBoundingBoxVertices(
    ...
    const normalizedVertices = [
      new vec2(Math.max(0, Math.min(1, centerX - halfWidth)), Math.max(0, Math.min(1, centerY - halfHeight))), // top-left
      new vec2(Math.max(0, Math.min(1, centerX + halfWidth)), Math.max(0, Math.min(1, centerY - halfHeight))), // top-right
      new vec2(Math.max(0, Math.min(1, centerX - halfWidth)), Math.max(0, Math.min(1, centerY + halfHeight))), // bottom-left
      new vec2(Math.max(0, Math.min(1, centerX + halfWidth)), Math.max(0, Math.min(1, centerY + halfHeight))) // bottom-right
    ]
    ...
    if (this.debug) {
      ...
      print(`Scaled dimensions: ${scaledWidth.toFixed(3)} x ${scaledHeight.toFixed(3)}`)
  ...
  private shouldUpdateSeamlessDetection(
    ...
    if (newVertices && lastVertices && newVertices.length === lastVertices.length) {
      for (let i = 0; i < newVertices.length; i++) {
        const vertexDistanceChanged = newVertices[i].distance(lastVertices[i]) * 100 // Convert to cm
  ...
  private initializeStaticScene(): void {
    ...
    if (this.debug) {
      print(`Static scene initialized with ${this.preInstantiatedDetections.length} pre-instantiated prefabs`)
    }
  }
  ...
  private startLerpTransition(
    ...
    if (this.debug) {
      print(`Started lerp transition for detection ${detectionIndex} (duration: ${this.lerpDuration}s)`)
    }
  }
