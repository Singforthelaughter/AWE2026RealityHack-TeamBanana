...
export class MapComponent extends BaseScriptComponent {
   ...
   */
  createMapPin(longitude: number, latitude: number, label?: string): MapPin {
    const location = GeoPosition.create()
    location.longitude = longitude
    location.latitude = latitude
    if (label !== undefined) {
      const placeInfo = { name: label } as PlaceInfo
      return this.mapController.createMapPin(location, placeInfo)
    }
