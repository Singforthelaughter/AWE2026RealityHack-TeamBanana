...
export class MapContainerController extends BaseScriptComponent {
  ...
  private onStart() {
    ...
    this.containerYOffset = this.container.getParentTransform().getWorldPosition().y - this.cameraTransform.getWorldPosition().y

    this.containerTransform = this.container.getParentTransform()
  ...
  private updateSmoothFollow() {
    if (!this.dragging) {
      ...
      let newY
      ;[pos.x, this.velocity.x] = smoothDamp(pos.x, this.target.x, this.velocity.x, this.translationXTime, getDeltaTime())
      ;[newY, this.velocity.y] = smoothDamp(currentY, targetY, this.velocity.y, this.translationYTime, getDeltaTime())
      ;[pos.z, this.velocity.z] = smoothDamp(pos.z, this.target.z, this.velocity.z, this.translationZTime, getDeltaTime())

      const worldXZPos = this.bodyToWorld(this.cylindricalToCartesian(pos))
      this.worldPos = new vec3(worldXZPos.x, newY, worldXZPos.z)
      ;[this.heading, this.omega] = smoothDampAngle(this.heading, this.cameraHeading, this.omega, this.rotationTime, getDeltaTime())
      // force billboard
  ...
  private handleMiniMapToggled(isMiniMap: boolean) {
    ...
    if (isMiniMap) {
      ...
      const targetWorldPosition: vec3 = containerWorldPosition.sub(this.cameraPos).normalize().uniformScale(CONTAINER_DISTANCE_MINI).add(this.cameraPos)
      ...
      const targetWorldPosition: vec3 = containerWorldPosition.sub(this.cameraPos).normalize().uniformScale(CONTAINER_DISTANCE_FULL).add(this.cameraPos)
  ...
  private clampPosition(): void {
    ...
    this.target.z = Math.max(this.target.z, (1.1 * this.visibleWidth) / 2 / Math.tan((this.fieldOfView / 2) * MathUtils.DegToRad)) // handle very wide panels
  ...
  private get halfFov(): number {
    ...
    return Math.atan((Math.tan((this.fieldOfView / 2) * MathUtils.DegToRad) * dist - this.visibleWidth / 2) / this.target.z)
  }
