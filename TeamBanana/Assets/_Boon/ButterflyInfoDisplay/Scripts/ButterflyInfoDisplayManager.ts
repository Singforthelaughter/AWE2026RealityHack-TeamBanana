...
export class ButterflyInfoDisplayManager extends BaseScriptComponent {
  ...
  public displayResult(suggestion: Suggestion, userPhotoTexture: Texture | null = null): void {
    ...
    this.populate({
      ...
      common_name: d?.common_names?.[0] ? d.common_names[0] : "",
      common_names: d?.common_names?.length ? "Common names: " + d.common_names.join(", ") : "",
      probability: "Probability: " + Math.round(suggestion.probability * 100) + "%",
      description: d?.description?.value ?? d?.description_gpt ?? "",
      inaturalist_id: d?.inaturalist_id != null ? "iNaturalist id: " + String(d.inaturalist_id) : "",
      red_list: d?.red_list ? "Red list: " + d.red_list : "",
      danger: d?.danger?.join(", ") ?? "",
      danger_description: d?.danger_description ? "Danger description: " + d.danger_description : "",
      role: d?.role?.length ? "Role: " + d.role.join(", ") : "",
      gbif_id: d?.gbif_id != null ? "GBIF id: " + String(d.gbif_id) : "",
      phylum: d?.taxonomy?.phylum ? "Phylum: " + d.taxonomy.phylum : "",
      class: d?.taxonomy?.class ? "Class: " + d.taxonomy.class : "",
      order: d?.taxonomy?.order ? "Order: " + d.taxonomy.order : "",
      family: d?.taxonomy?.family ? "Family: " + d.taxonomy.family : "",
      genus: d?.taxonomy?.genus ? "Genus: " + d.taxonomy.genus : "",
      last_seen: "Last seen: " + new Date().toISOString(),
      seen_by: "Seen by: " + this.snapDisplayName,
    })
  ...
  public async displaySighting(sighting: SightingInfo): Promise<void> {
    ...
    this.populate({
      ...
      common_name: sighting.speciesCommonNames?.[0] ? sighting.speciesCommonNames[0] : "",
      common_names: sighting.speciesCommonNames?.length ? "Common names: " + sighting.speciesCommonNames.join(", ") : "",
      probability: sighting.speciesProbability != null ? "Probability: " + Math.round(sighting.speciesProbability * 100) + "%" : "",
      description: sighting.speciesDescription ?? "",
      red_list: sighting.speciesRedList ? "Red list: " + sighting.speciesRedList : "",
      danger: sighting.speciesDanger?.join(", ") ?? "",
      danger_description: sighting.speciesDangerDescription ? "Danger description: " + sighting.speciesDangerDescription : "",
      role: sighting.speciesRole?.length ? "Role: " + sighting.speciesRole.join(", ") : "",
      phylum: tax?.phylum ? "Phylum: " + tax.phylum : "",
      class: tax?.class ? "Class: " + tax.class : "",
      order: tax?.order ? "Order: " + tax.order : "",
      family: tax?.family ? "Family: " + tax.family : "",
      genus: tax?.genus ? "Genus: " + tax.genus : "",
      spotter: sighting.snapDisplayName ? "Spotter: " + sighting.snapDisplayName : "",
      last_seen: sighting.identifiedAt ? "Last seen: " + sighting.identifiedAt : "",
      seen_by: sighting.snapDisplayName ? "Seen by: " + sighting.snapDisplayName : "",
    })
