...
/**
 * Local type definitions matching ChatComponent's internal CardData/CardType.
 * ChatComponent defines these at module scope; we match the shape here.
 */
const enum CardType {
  User = 0,
  Chatbot = 1
  ...
  public static addUserCard(agenticChat: ChatComponent, message: string): boolean {
    ...
    try {
      ...
      const cardData: CardData = {
        ...
        type: CardType.User,
        textContent: message,
        size: new vec3(25, 5 + Math.ceil(message.length / 45) * 1.5, 3),
        sceneObject: null
      }
