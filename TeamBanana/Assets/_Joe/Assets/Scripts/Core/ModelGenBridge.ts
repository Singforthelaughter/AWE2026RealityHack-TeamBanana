...
/**
 * ModelGenBridge - Bridge component for 3D model generation
 *
...
export class ModelGenBridge extends BaseScriptComponent {
  ...
  private initializeFactory(): void {
    if (!this.modelGenFactory) {
      if (this.enableDebugLogging) {
        print("ModelGenBridge: ModelGen factory not assigned - skipping initialization")
      }
  ...
  public async generateModel(prompt: string, controlSpinner: boolean = true): Promise<void> {
    ...
    if (controlSpinner) {
      if (this.spinner) {
        ...
        print(`ModelGenBridge: Loading spinner not assigned (standalone mode)`)
      }
    } else {
      print(`ModelGenBridge: Spinner control delegated to calling Node component`)
    }
    ...
    try {
      ...
      if (!controlSpinner && this.onExternalSuccess) {
        const safetyTimeout = setTimeout(() => {
          ...
          if (this.isGenerating && !this.callbackInvoked) {
            print(`ModelGenBridge: Safety timeout - ensuring spinner is disabled`)
            this.callbackInvoked = true // Prevent double callback
      ...
      if (controlSpinner && this.spinner) {
        this.spinner.enabled = false
        print(`ModelGenBridge: Loading spinner disabled (standalone mode)`)
      }
    }
  }
  ...
  public clearModels(): void {
    ...
    if (this.enableDebugLogging) {
      print("ModelGenBridge: All models cleared")
    }
  }
