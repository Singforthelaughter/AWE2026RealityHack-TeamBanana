...
export class GeneralConversationTool {
  ...
  public async execute(args: Record<string, unknown>): Promise<{success: boolean; result?: any; error?: string; executionTime: number}> {
    const startTime = Date.now()
    const {query} = args

    if (!query || typeof query !== "string") {
      return {success: false, error: "Query parameter is required and must be a string", executionTime: Date.now() - startTime}
    }

    const result = await this.generateConversationalResponse(args)
