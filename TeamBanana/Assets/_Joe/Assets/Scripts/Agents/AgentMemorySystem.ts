 ...
 * Stores conversation history and summaries
...
export class AgentMemorySystem {
  ...
  constructor() {
    try {
      ...
      if (typeof global !== "undefined" && global.persistentStorageSystem) {
        ...
        print("AgentMemorySystem: PersistentStorage not available, using in-memory storage")
  ...
  public addMessage(message: Message): void {
    ...
    if (this.messages.length > 500) {
      ...
      print("AgentMemorySystem: Message history truncated to prevent memory overflow")
    }

    this.saveToStorage()
    print(`AgentMemorySystem: Added message: ${message.role} - ${message.content.substring(0, 50)}...`)
  }
  ...
  public addChatMessage(message: ChatMessage): void {
    ...
    if (this.chatHistory.length > 100) {
      ...
      print("AgentMemorySystem: Chat history truncated to prevent memory overflow")
    }

    this.saveToStorage()
    print(`AgentMemorySystem: Added chat message: ${message.type} - ${message.content.substring(0, 50)}...`)
  }
  ...
  public clearStorage(): void {
    try {
      if (!this.persistentStorage) {
        print("AgentMemorySystem: No persistent storage available to clear")
        this.initializeMemory()
        return
      }
  ...
  private initializeMemory(): void {
    ...
    print("AgentMemorySystem: Memory initialized")
  }
  ...
  private saveComponent(key: string, data: any): void {
    ...
    if (data !== null && data !== undefined) {
      ...
      if (size > this.MAX_STORAGE_SIZE / 4) {
        // Don't let any single component exceed 25% of storage
        print(`AgentMemorySystem: Component ${key} exceeds size limit, truncating...`)
      ...
      try {
        ...
        print(`AgentMemorySystem: Failed to save component ${key}: ${error}`)
      }
    }
  }
  ...
  private loadComponent<T>(key: string, defaultValue: T): T {
    try {
      ...
      print(`AgentMemorySystem: Failed to load component ${key}: ${error}`)
    }
    return defaultValue
  }
  ...
  private getStorageMetadata(): {size: number; lastUpdated: number} {
    try {
      ...
      print(`AgentMemorySystem: Failed to get storage metadata: ${error}`)
    }
