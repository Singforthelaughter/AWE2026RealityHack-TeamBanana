 ...
 * context across interactions. The user's default option is to save chat flow
...
export class ChatStorage extends BaseScriptComponent {
  ...
  @input
  @hint("Default option to save sessions")
  ...
  public endCurrentSession(): ChatSession | null {
    if (!this.currentChatSession) {
      if (this.enableDebugLogging) {
        print("ChatStorage: No active session to end")
      }
      return null
    }
  ...
  public getAllSessions(): ChatSession[] {
    return Array.from(this.storedSessions.values()).sort((a, b) => b.lastActivity - a.lastActivity)
  }
  ...
  /**
   * Clear all memory - implements the "checkbox to delete memory"
   */
  public clearAllMemory(): void {
    ...
    if (this.enableDebugLogging) {
      print("ChatStorage: All chat memory cleared (user requested memory deletion)")
    }
  }
  ...
  public clearSession(sessionId: string): boolean {
    if (this.storedSessions.has(sessionId)) {
      ...
      if (this.enableDebugLogging) {
        print(`ChatStorage: Session ${sessionId} cleared`)
      }
      return true
    }
    return false
  ...
  private cleanupOldSessions(): void {
    ...
    const sessionsArray = Array.from(this.storedSessions.entries()).sort(
    ...
    if (this.enableDebugLogging && toRemove.length > 0) {
      print(`ChatStorage: Cleaned up ${toRemove.length} old sessions`)
    }
  }
